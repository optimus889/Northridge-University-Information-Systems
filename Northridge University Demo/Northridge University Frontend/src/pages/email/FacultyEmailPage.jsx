...
export default function FacultyEmailPage() {
  ...
  useEffect(() => {
    emailApi.getFacultyEmails().then((data) => {
      setEmails(data);
  ...
  async function handleSend(event) {
    ...
    const result = await emailApi.sendFacultyEmail({
      from: user?.email || "m.chen@northridge.edu",
      ...compose
    });
    ...
    <PortalLayout>
      <section className="subpage-header">
        <div>
          ...
          <h1>Faculty Email</h1>
          <p>
            View department messages and prepare secure faculty email
            communications.
          </p>
        </div>
      ...
      <section className="panel email-compose-panel">
        ...
        <form className="email-compose-form" onSubmit={handleSend}>
          <label>
            From
            <input
              value={user?.email || "m.chen@northridge.edu"}
              disabled
            />
          </label>
