...
export default function StudentEmailPage() {
  ...
  useEffect(() => {
    emailApi.getStudentEmails().then((data) => {
      setEmails(data);
  ...
  async function handleSend(event) {
    ...
    const result = await emailApi.sendStudentEmail({
      from: user?.email || "e.carter@northridge.edu",
      ...compose
    });
    ...
    <PortalLayout>
      <section className="subpage-header">
        <div>
          ...
          <h1>Student Email</h1>
          <p>
            View university messages and prepare secure student email
            communications.
          </p>
        </div>
      ...
      <section className="panel email-compose-panel">
        ...
        <form className="email-compose-form" onSubmit={handleSend}>
          <label>
            From
            <input
              value={user?.email || "e.carter@northridge.edu"}
              disabled
            />
          </label>
